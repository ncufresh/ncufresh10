...
if(isset($_POST['submit']) && $_POST['submit'] == GUI_ADD_LINK_TEXT)
{
	...
	{
		$currdb -> sql_query("UPDATE `anno_topics` SET `title`='".$_POST['title']."', `uid`='".($curruser->uid)."', `datetime`='".mktime()."', `contents`='".$_POST['contents']."', `ip_addr`='".get_client_ip_addr()."' WHERE `tid`='".intval($_POST['tid'])."'");
		$curr_tid = intval($_POST['tid']);
	}
	else
	if($_GET['action'] == "op_new")
	{
		$currdb -> sql_query("INSERT INTO `anno_topics` (`title`, `uid`, `datetime`, `contents`, `ip_addr`, `is_display`) VALUES ('".$_POST['title']."', '".($curruser->uid)."', '".mktime()."', '".$_POST['contents']."', '".get_client_ip_addr()."', '0')");
...
if(isset($_POST['submit']) && $_POST['submit'] == GUI_UPLOAD_TEXT)
{
	...
	$date_prefix = date("Ymd_His", mktime());
...
switch($_GET['action'])
{
	...
	case "op_new":
		...
		else
		{
			if($_GET['action'] == "op_edit")
			{
				$currdb -> sql_query("UPDATE `anno_topics` SET `title`='".$_POST['title']."', `uid`='".($curruser -> uid)."', `datetime`='".mktime()."', `contents`='".$_POST['contents']."', `ip_addr`='".get_client_ip_addr()."', `is_display`='".$_POST['is_display']."' WHERE `tid`='".intval($_POST['tid'])."'");
				
				redirect("list_anno.php?action=update_success");
			}
			else
			{
				$currdb -> sql_query("INSERT INTO `anno_topics` (`title`, `uid`, `datetime`, `contents`, `ip_addr`, `is_display`) VALUES ('".$_POST['title']."', '".($curruser -> uid)."', '".mktime()."', '".$_POST['contents']."', '".get_client_ip_addr()."', '".$_POST['is_display']."')");
